<script lang="ts">
  ...
  import { getResultFile, getAllResults } from "../store.svelte";
  ...
  // Create preview props for result file (and related files for markdown/images)
  const previewProps = $derived((): Record<string, unknown> => {
    const result = selectedResult();
    if (!result?.content) return {};

    // Primary file for preview
    const baseFile = new File([result.content], result.filename, {
      type: getMimeType(result.filename),
      lastModified: new Date(result.createdAt).getTime()
    });

    // For markdown previews, provide related files generated by the same script (e.g., assets/...)
    let relatedFiles: Array<{ path: string; file: File }> = [];
    const isMarkdown = result.filename.toLowerCase().endsWith('.md');
    if (isMarkdown) {
      const all = getAllResults();
      relatedFiles = all
        .filter(r => r.id !== result.id && r.scriptId === result.scriptId && r.content)
        .map(r => {
          const f = new File([r.content as BlobPart], r.filename, {
            type: getMimeType(r.filename),
            lastModified: new Date(r.createdAt).getTime()
          });
          return { path: r.filename, file: f };
        });
    }
    ...
    return {
      ...
      // Pass related files for markdown so relative links can resolve
      ...(isMarkdown ? { relatedFiles } : {})
    };
  });
